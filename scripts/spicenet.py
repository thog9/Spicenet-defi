...
LANG = {
    'vi': {
        ...
        'token_expired': '⚠ Token đã hết hạn tại {expires_at}. Vui lòng tạo token mới bằng spicenet_pvkey.py!',
    ...
    'en': {
        ...
        'token_expired': '⚠ Token expired at {expires_at}. Please generate a new token using spicenet_pvkey.py!',
...
async def process_token(index: int, token: str, task_type: str, language: str = 'vi', proxies: List[str] = None) -> bool:
    ...
    expires_at = user_info.get("twitterMetadata", {}).get("tokenExpiresAt")
    if expires_at:
        expires_at_dt = datetime.fromisoformat(expires_at.replace("Z", "+00:00"))
        now_dt = datetime.now(UTC)
        if now_dt > expires_at_dt:
            print(f"{Fore.YELLOW} ⚠ {LANG[language]['token_expired'].format(expires_at=expires_at_dt.strftime('%Y-%m-%d %H:%M:%S %Z'))}{Style.RESET_ALL}")
            return False

    user_id = user_info.get("twitterMetadata", {}).get("userId", "Unknown")
    ...
    print(f"{Fore.YELLOW}    - {LANG[language]['user_info'].format(name=user_info.get('twitterMetadata', {}).get('name', 'Unknown'), username=user_info.get('twitterMetadata', {}).get('username', 'Unknown'))}")
    print(f"{Fore.YELLOW}    - {LANG[language]['location'].format(location=user_info.get('twitterMetadata', {}).get('location', 'Unknown'))}")
    print(f"{Fore.YELLOW}    - {LANG[language]['token_display'].format(token=token[:20] + '...' + token[-10:])}")
    ...
    for task_guid in tasks_to_complete:
        ...
        if not task_info.get("isEnabled", False):
            print(f"{Fore.YELLOW} ⚠ {LANG[language]['locked'].format(task=task_info.get('title', task_guid))}{Style.RESET_ALL}")
            continue
        if task_info.get("pointsReceived", "0") != "0" or task_info.get("title", "") == "Follow Spicenet on X":
            ...
            print(f"{Fore.YELLOW}   - {LANG[language]['task_guid'].format(guid=task_guid)}")
            continue
...
def select_task_type_for_script(language: str = 'vi'):
    messages = {
        'vi': {
            'select_task_type': 'Chọn loại nhiệm vụ để thực hiện',
            'menu_title': 'MENU NHIỆM VỤ SPICENET'
        },
        'en': {
            'select_task_type': 'Select task type to execute',
            'menu_title': 'SPICENET TASKS MENU'
        }
    }
    print_border(messages[language]['menu_title'], Fore.YELLOW)
    print(f"{Fore.CYAN}│ {messages[language]['select_task_type'].center(BORDER_WIDTH - 4)} │{Style.RESET_ALL}")
    questions = [
        inquirer.List('task_type',
                      message=f"{Fore.CYAN}{messages[language]['select_task_type']}{Style.RESET_ALL}",
                      choices=[
